...
- **`PushX.WebPush.generate_vapid_keys/0`** and **`validate_subscription/1`** for programmatic use; `PushX.validate_token(:webpush, subscription)` and `push_batch(:webpush, ..., validate_tokens: true)` run the same local check. `PushX.Message.to_webpush_options/1` (`ttl` → `TTL`, `priority` → `Urgency`) is applied automatically when a `Message` is sent, like `to_apns_options/1` for APNS. **`PushX.Test.webpush_subscription/1`** builds a valid subscription fixture for test delivery mode.
- `:webpush` is a reserved instance name (like `:apns`/`:fcm`); `PushX.JWTCache.invalidate_match/1`.

- **HTTP/2 keepalive and connection recycling (finch ≥ 0.22)** — new config `finch_http2_ping_interval` (default **60 000** ms: an HTTP/2 PING after a minute of idleness keeps APNS/FCM connections alive through the idle gaps after which cloud load balancers silently drop them, and detects a dead one before a real send pays for it with a failed attempt), `finch_http2_max_connection_age` (+ `finch_http2_max_connection_age_jitter`) to drain and replace connections on a schedule (rotating DNS / LBs; default `:infinity`), and `finch_http2_wait_for_server_settings` (default `false`). Applied to every HTTP/2 pool (APNS prod/sandbox, FCM, Instance ID) and to named instances, which can override per instance with `ping_interval:` / `max_connection_age:` / `max_connection_age_jitter:` / `wait_for_server_settings:` in their config. On finch < 0.22 the options are ignored with a boot-time warning. Prompted by a production trace (Fly.io, 12 minutes idle → dead socket → two retry attempts before delivery) where recovery-by-retry was the wrong place to pay for an idle connection.
...
### Changed
- **Pool-sizing guidance corrected** — for HTTP/2 pools Finch ignores `:size`; **`finch_pool_count` is the number of HTTP/2 connections per origin** and the capacity/redundancy knob for APNS/FCM. The README, `PushX.Config` docs, AGENTS.md and `mix pushx.doctor` now say so (the doctor warns on `finch_pool_count: 1`), replacing the earlier advice to "lower `finch_pool_size` for low-traffic apps", which had no effect on APNS/FCM and, when people also dropped `finch_pool_count` to 1, left a single socket that a retry burst then saturated.
- **Connection-error backoff uses full jitter** (uniform between half and the full exponential window) so a burst of retries — e.g. every task of a batch after a pool reconnect — spreads out instead of converging on the freshly opened connection's stream limit.
- **`too_many_concurrent_requests` / `connection_not_ready` are explained in the logs** (`PushX.HTTP`): still classified `:connection_error` and retried, but the log line says it is pool capacity, not a network failure, and points at `:finch_pool_count` / batch `:concurrency`.
...
- The shared Finch: `:http2` pool options are only passed on finch ≥ 0.22 (an app still on finch 0.21 boots and warns once instead of failing with `unknown options [:http2]`), and the HTTP/1.1 `:default` pool (Web Push) now honours `:connect_timeout`, uses `count: 1` (`finch_pool_size` is its per-origin cap) and a pool checkout timeout surfaces as a retryable `:connection_error` (`pool_timeout`) instead of a raise. Web Push connection errors do **not** restart the pool (`Retry` `reconnect: false`) — a dead third-party push-service endpoint must not tear down the APNS/FCM HTTP/2 connections.
...
- **`PushX.Instance.Loader`** — a supervision-tree child (`{PushX.Instance.Loader, instances: fun_or_list_or_mfa}`) that starts named instances on boot from your own source of truth, synchronously (it runs `load/1` in `start_link/1` and returns `:ignore`, so later children see the instances). Already-running names are reported, not restarted; one tenant's bad credentials are logged and skipped unless `on_error: :raise`. `load/1` is public for re-runs and returns `%{started, already_running, failed}`.
- **`mix pushx.doctor`** — checks the configuration offline with exactly the checks the library runs (APNS key resolves and signs ES256, FCM service account resolves and signs RS256 or the token fetcher is exported, project id present, retry/pool/breaker/limiter/cleanup summary, delivery-mode warning); raises `Mix.Error` (non-zero exit) on a credential problem so it can gate a deploy. Nothing is sent.
...
### Fixed (pre-release review of 0.14)
- Test delivery mode is now fully isolated from production machinery: the circuit breaker and rate limiter are neither consulted nor fed (a stubbed failure can't open the global breaker for other tests); FCM instances start no Goth process in test mode (the documented `PushX.Test.fcm_credentials/0` recipe no longer contacts Google's OAuth endpoint); recorded pushes and stubs are deleted automatically when the owning test process exits (`PushX.Test.Store` monitors owners), so memory is bounded and pid reuse can't surface stale records; an invalid `:retry` value fails in test mode exactly as in production (validated before the test-mode short-circuit).
- FCM topic requests on a named instance now reconnect *that* instance's pool on connection errors (previously the static `PushX.Finch` pool was restarted, killing unrelated in-flight sends while the failing pool kept its dead sockets), and instance/static Finch configurations include an HTTP/2 pool for the Instance ID origin so instance sizing/timeouts apply to topic calls. Instance ID API string errors (`{"error": "InvalidToken"}`) are mapped by HTTP status with Google's reason kept.
- Atom-keyed `:apns` / `:android` overrides are key-normalised before being merged with `Message`-derived maps (previously both `"payload"` and `:payload` could reach the JSON).
- `telemetry_metrics` optional dependency accepts `~> 0.6 or ~> 1.0` so apps on the 0.6 line can install PushX; `Config` moduledoc states the real rate-limit default (5 000); `Telemetry.metrics/0` docs describe `pushx.push.duration` (successes) and `pushx.push.error.duration` (failures — alert on this one) accurately.
...
[Unreleased]: https://github.com/cignosystems/pushx/compare/v0.15.0...HEAD
